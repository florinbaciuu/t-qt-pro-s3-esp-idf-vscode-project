...
TaskHandle_t xHandle_TaskManager_Process;
...
//
        ...
        ESP_LOGI("LVGL", "Free INTERNAL memory: %u bytes", heap_caps_get_free_size(MALLOC_CAP_INTERNAL));
        ESP_LOGI("LVGL", "Free DMA memory: %u bytes", heap_caps_get_free_size(MALLOC_CAP_DMA));
        ...
        xTaskCreatePinnedToCore(
            ...
            (uint32_t)(2048 + 0),                // Dimensiunea stack-ului
            (NULL),                              // Parametri
            // (2),                                  // Prioritatea task-ului
